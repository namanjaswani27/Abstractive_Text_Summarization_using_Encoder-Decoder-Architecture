...
class Decoder(nn.Module):
    ...
    num_layers = 1, dropout = 0.1):
        ...
        self.context_weights = nn.Linear(6*hidden_size, 2*hidden_size)
    ...
    prev_target_embedding):
        ...
        # Attention
        # context1: [ batch_size, 1, 2 * hidden_size ]
        context1 = self.attention1(encoder_output = encoder_output, 
        decoder_hidden = decoder_hidden[-1].unsqueeze(1))

        # context2: [ batch_size, 1, 2 * hidden_size ]
        context2 = self.attention2(encoder_output = encoder_output, 
        decoder_hidden = decoder_hidden[-1].unsqueeze(1))

        # context3: [ batch_size, 1, 2 * hidden_size ]
        context3 = self.attention3(encoder_output = encoder_output, 
        decoder_hidden = decoder_hidden[-1].unsqueeze(1))

        # context: [ batch_size, 1, 6 * hidden_size ]
        context = torch.cat([context1, context2, context3], dim=2)

        # context: [ batch_size, 1, 2 * hidden_size ]
        context = self.context_weights(context)

        # Teacher Forcing
        # decoder_input: [ batch_size, 1, embedding_size + 6 * hidden_size ]
