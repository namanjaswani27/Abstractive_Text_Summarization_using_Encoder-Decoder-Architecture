...
class Decoder(nn.Module):
    ...
    num_layers = 1, dropout = 0.1):
        ...
        self.encoder_attention = Attention(output_size = 2*hidden_size, 
        hidden_size = hidden_size)

        self.decoder_attention = Attention(output_size = hidden_size, 
        hidden_size = hidden_size)
    ...
    prev_target_embedding, decoder_outputs):

        """Perform a single decoder step (1 word)"""

        # Attention
        # context: [ batch_size, 1, 2 * hidden_size ]
        context = self.encoder_attention(output = encoder_output, 
        hidden = decoder_hidden.transpose(0, 1))

        if(len(decoder_outputs) > 0 ):
            decoder_hidden = self.decoder_attention(
                output = torch.cat(decoder_outputs, dim=1), 
                hidden = decoder_hidden.transpose(0, 1)).transpose(0, 1)
    ...
    decoder_hidden = None):
        ...
        batch_size, target_len, _ = target_embedding.size()
